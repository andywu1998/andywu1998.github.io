...
> 来源：`notes/documents/01_flomo_整合进个人助理方案.md`
# flomo 整合进个人助理方案
...
读取能力已实测打通：官方移动端接口 `POST /api/v1/user/login_by_email` 配合
`GET /api/v1/memo/updated/` 可以稳定读取全量 memo（本次 513 条），网页端登录路由
`/api/v1/user/web/login_by_email` 则被风控 429。导出结果、签名算法与工具见
[flomo 笔记全量结构化导出到本地](01_flomo_笔记全量结构化导出到本地.md)。
